...
let Container = React.createClass({
	...
	render() {
		...
		return <div className="container">
					<DropDown value={this.getYearData()[0]} options={this.getYearData()} onSelect={yearFilterHandler}/>
					<div>{this.renderChart()}</div>
					<div className="datagrid">{this.renderDataGrid()}</div>
					</div>
	},
	renderChart() {
		...
		return <Chart year={this.state.year} data={this.getFormattedChartData()} onClick={onClick}/>
	},
	renderDataGrid() {
